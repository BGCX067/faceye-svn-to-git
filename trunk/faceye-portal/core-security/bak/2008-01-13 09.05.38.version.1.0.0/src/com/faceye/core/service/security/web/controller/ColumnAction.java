	...
	public ActionForward tree(ActionMapping mapping, ActionForm form,
		...
		String json="";
			if (super.getHttp().getParameter(request, "node").equals("source")) {
				json=super.getService().getTreeService().treeJSON(transferColumns);
			} else {
				String currentNode = super.getHttp().getParameter(request,
						"node");
				json = this.getService().getTreeService().treeJSON(
						transferColumns,
						currentNode);
			}
 ...
 */
	...
	public ActionForward columns(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response){
	    ...
	    if(StringUtils.isNotEmpty(columnId)){
	    	transferColumns=this.getService().getTreeService().getDirectChildrenTrees(this.getColumnService().transferColumns(columns), columnId);
	    }else{
	    	transferColumns=this.getService().getTreeService().getRoots(this.getColumnService().transferColumns(columns));
	    }
