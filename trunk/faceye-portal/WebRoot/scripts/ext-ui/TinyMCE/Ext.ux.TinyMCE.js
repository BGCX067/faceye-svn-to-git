		...
		// Members
		{
			...
			*/        
			onRender : function( ct, position ){ 
				...
				this.ed.windowManager = new WindowManager( this.ed );
			...
			onResize : function( aw, ah ){
				if( this.rendered ){
					this.withEd( function() {
						...
						Ext.fly( this.ed.id + "_parent" ).setSize( aw, ah );
						var t = Ext.get( this.ed.id + "_tbl" );
						t.setSize( aw - 2, ah - 2 );
						
						var tbar = t.select( "td.mceToolbar > div" ).first();
						var h = 5;
						if( tbar ) { 
							h += tbar.getHeight() + 2;
							tbar.setWidth( aw - 2 );
						...
						t.select( "td.mceIframeContainer > iframe" ).first().setHeight( ah - h );
					});
				}
			},
