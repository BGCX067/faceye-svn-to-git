	...
	init : function(id, name) {
		...
		var store = new Ext.data.Store({
			proxy : new Ext.data.HttpProxy({
				url : BP + 'portalContainerAction.do?method=getNewerPortalContainer'
			}),
...
function renderTopic(value, metadata, record, rowIndex, colIndex, store) {
			...
			return String.format(html, record.data.username, record.data.createTime,record.data.username);
		}
		...
		var grid = new Ext.grid.GridPanel({
			...
			// title : '我的博客',
			id : 'user-blog-newer-list-grid_'+Ext.id,
			header : false,
			region : 'center',
			border : false,
			autoHeight : true,
