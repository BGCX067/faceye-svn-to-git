...
var TreeController={
    ...
    var store = new Ext.data.Store({
        ...
        proxy: new Ext.data.HttpProxy({
            url: BP+'treeAction.do?method=index'
        }),
...
//                        totalProperty: 'total',
                        success:true,
                       fields: [
                             'id','name','action','url','parentId','parentName'
                            ]
                        }
                     ),
                    items: [
...
//                                    grid.getView().refresh();	
                           			...
                           			updateWin.hide();
                           			//重新加载数据到grid
                           			Ext.getCmp('default-manager-tree').root.reload();
                           			store.load();
                           		},
                           		failure:function(){
                           			Ext.Msg.alert('节点保存','节点保存失败!');
            		...
            		updateForm.getForm().load({
            	    	url:BP+'treeAction.do?method=update&entityClass=com.faceye.core.service.security.model.Tree&id='+id,
            	    	waitMsg:'正在加载数据，请稍后...'
            	    });
                    updateWin.show(this);
//                    loadStore.load();
...
//                                 )
            		...
            		nodesStore.load({
            			...
            			callback:function(){
            		        ...
            		        //发送删除数据的请求		
            		       Ext.Ajax.request({
            			     url:BP+'treeAction.do?method=remove',
            			     failure:function(){
...
//                                 )
                         ...
                         root:'rows',
                         fields:['id','name','action','url','parentId','parentName']
            		});
