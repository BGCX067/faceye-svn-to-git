com.faceye.compoents.core.security.SelectRoles ={
...
//	    }
      ...
      var fromStore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: BP+'roleAction.do?method=getRolesForUserPermission&exists=false&userId='+userId
        }),
       ...
       */
        ...
        proxy: new Ext.data.HttpProxy({
            url: BP+'roleAction.do?method=getRolesForUserPermission&exists=true&userId='+userId
        }),
        reader: new Ext.data.JsonReader({
            root: 'root',
            totalProperty: 'total',
      ...
      toStore.load();
		var formItemSelector =new Ext.form.FormPanel({ 
				...
				labelAlign:'top',
				width:450,
				url:BP+'userAction.do?method=permission',
//				margins:'50 50 50 50',
//                autoScroll:true,
                bodyStyle:'padding:5px 5px 5px 5px',
				items:[{
					xtype:"itemselector",
					url:BP+'userAction.do?method=permission&userId='+userId,
					name:"roleIds",
//                    autoScroll:true,
					fieldLabel:"为用户授权",
...
//					toData:[["10", "Ten"]],
					...
					toTBar:[{
						text:"清除",
						handler:function(){
							var i=formItemSelector.getForm().findField("roleIds");
							i.reset.call(i);
						}
					}]
				}]
