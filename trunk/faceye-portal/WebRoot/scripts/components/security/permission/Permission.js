...
com.faceye.compoents.core.security.Permission={
	init:function(){
    ...
    var store = new Ext.data.Store({
        ...
        proxy: new Ext.data.HttpProxy({
            url: BP+'permissionAction.do?method=index'
        }),
...
//        return String.format('{0}<br/>by {1}', value.dateFormat('M j, Y, g:i a'), r.data['lastposter']);
       return String.format('<a href="#" onclick="com.faceye.compoents.core.security.SelectResources.init(\'{0}\')">授权</a>',r.get('id'));
    }
        ...
        tbar:[{
            ...
            　　
                 ...
                 var innerForm=new Ext.FormPanel({
                    ...
                    url:BP+'permissionAction.do?method=save',
                    frame:true,
                    // title: 'Simple Form',
...
//            	    innerForm.addListener('click', function(){alert('keup');});
            		...
            		win=new Ext.Window({
                        ...
                        buttons: [{
                           text:'确定',
                           scope:com.faceye.compoents.core.security.Permission,
                           type:'submit',
                           disabled:false,
                           handler:function(){
                           innerForm.getForm().submit({
                           		method:'POST',
                           		params:{
                           		entityClass:'com.faceye.core.service.security.model.Permission'	
                           		},
                           		waitMsg:'正在保存数据...',
                           		//reset:BP+'permissionAction.do?method=generatorJSONList&entityClass=com.faceye.core.componentsupport.dao.model.DataType',
                           		success:function(form,action){
//                                    grid.getView().refresh();
                           			form.reset();
                           			Ext.Msg.alert('权限保存','权限保存成功!');
                           			this.disabled=false; 
                           			win.hide();
                           			//重新加载数据到grid
                           			store.load();
...
//                                    grid.getView().refresh();	
            		...
            		updateForm.getForm().load({
            	    	url:BP+'permissionAction.do?method=update&entityClass=com.faceye.core.service.security.model.Permission&id='+id,
            	    	waitMsg:'正在加载数据，请稍后...'
            	    });
                    updateWin.show(this);
//                    loadStore.load();
            	...
            	if(selectedCount==0){
            		...
            		for(var i=0;i<records.length;i++){
            		    ...
            		    _ids+='_';
            		        ...
            		        //发送删除数据的请求		
            		       Ext.Ajax.request({
            			     url:BP+'permissionAction.do?method=remove',
            			     failure:function(){
            				 Ext.Msg.alert('权限删除','权限删除失败！');
            			    },
            			   success:function(){
            				 Ext.Msg.alert('权限删除','权限删除成功！');
            			    },
            			   params:{
            				entityClass:'com.faceye.core.service.security.model.Permission',
            				ids:_ids
...
//            			 proxy: new Ext.data.HttpProxy({
//                                  url: BP+'permissionAction.do?method=isLeaf'
//                                }),
//            			 reader: new Ext.data.JsonReader({
//                                  root: 'rows',
//                                  fields: [
...
//                                  }
//                                 )
                         url:BP+'permissionAction.do?method=detail',
                         root:'rows',
                         fields:['id','name']
            		});
