...
com.faceye.compoents.core.security.Resource={
	init:function(){
    ...
    var store = new Ext.data.Store({
        ...
        proxy: new Ext.data.HttpProxy({
            url: BP+'resourceAction.do?method=index'
        }),
        ...
        reader: new Ext.data.JsonReader({
            ...
            id: 'id',
            fields: [
               'id','name','resourceType','resourceStr'
            ]
        }
        )
        ...
        tbar:[{
            ...
            　　
                 ...
                 var innerForm=new Ext.FormPanel({
                    ...
                    url:BP+'resourceAction.do?method=save',
                    frame:true,
                    // title: 'Simple Form',
...
//            	    innerForm.addListener('click', function(){alert('keup');});
            		...
            		win=new Ext.Window({
                        ...
                        buttons: [{
                           text:'确定',
                           scope:com.faceye.compoents.core.security.Resource,
                           type:'submit',
                           disabled:false,
                           handler:function(){
                           innerForm.getForm().submit({
                           		method:'POST',
                           		params:{
                           		entityClass:'com.faceye.core.service.security.model.Resource'	
                           		},
                           		waitMsg:'正在保存数据...',
                           		//reset:BP+'resourceAction.do?method=generatorJSONList&entityClass=com.faceye.core.componentsupport.dao.model.DataType',
                           		success:function(form,action){
//                                    grid.getView().refresh();
                           			form.reset();
                           			Ext.Msg.alert('资源保存','资源保存成功!');
                           			this.disabled=false; 
                           			win.hide();
                           			//重新加载数据到grid
                           			store.load();
...
//                                    grid.getView().refresh();	
            		...
            		updateForm.getForm().load({
            	    	url:BP+'resourceAction.do?method=update&entityClass=com.faceye.core.service.security.model.Resource&id='+id,
            	    	waitMsg:'正在加载数据，请稍后...'
            	    });
                    updateWin.show(this);
//                    loadStore.load();
...
//                    alert(loadRecord.name)
	        ...
	        //发送删除数据的请求		
            		       Ext.Ajax.request({
            			     url:BP+'resourceAction.do?method=remove',
            			     failure:function(){
            				 Ext.Msg.alert('资源删除','资源删除失败！');
            			    },
            			   success:function(){
            				 Ext.Msg.alert('资源删除','资源删除成功！');
            			    },
            			   params:{
            				entityClass:'com.faceye.core.service.security.model.Resource',
            				ids:_ids
    	...
    	if(!win){
    		win=new Ext.Window({
               ...
               var nodes=new Ext.data.JsonStore({
                         url:BP+'resourceAction.do?method=detail',
                         root:'rows',
                         fields:['id','name','resourceType','resourceStr']
            		});
